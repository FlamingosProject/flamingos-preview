...
- [Tutorial 12 - Integrated Testing](#tutorial-12---integrated-testing)
  ...
  - [Implementation](#implementation)
    ...
    - [Reusing QEMU Exit Codes](#reusing-qemu-exit-codes)
...
The Chapter 03 boot smoke test already taught the kernel to terminate QEMU through semihosting and
used the `test_build` feature to distinguish test behavior. This chapter reuses that mechanism for
the new custom unit and integration test runners.
...
The existing mechanism must now be wired into every new test entry and panic path. In
[@phil-opp]'s testing article, you [learned how to do this] for `x86 QEMU` systems by using a special
`ISA` debug-exit device. That device is not available on `AArch64`.

As in the boot test, we leverage ARM [semihosting] emulation and the [qemu-exit] crate. The shared
architecture code provides the following exit calls for all test binaries:
...
You might have also noted the `#[cfg(feature = "test_build")]`. As with `make test_boot`, the
`Makefile` enables this feature only for test artifacts. Testing-specific exit behavior is therefore
not compiled into normal kernels.
...
`assert!` macros. The boot smoke test already makes a `test_build` panic exit QEMU with failure. This
chapter carries that behavior into the shared kernel library so every unit and integration test
reports a panic without stalling:
...
This command launches QEMU with the same semihosting support used by `make test_boot` since
Tutorial 03. Unit and integration tests additionally route execution through
`ruby ../common/tests/dispatch.rb` so Cargo can run several kinds of test kernels.
