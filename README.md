...
- A fixed-size early console buffer retains diagnostics until the UART is ready.
...
driver has been initialized. The previous early console is a BSS-backed fixed-size buffer, so output
produced before that handoff is replayed instead of discarded. It requires neither a heap nor the
MMU and is therefore available during the earliest Rust initialization.
...
first driver added in these tutorials is the `PL011Uart` driver: It implements the raw console
traits, and is from now on used as the byte device below the terminal console. The terminal console
provides text-oriented newline handling and optional input echo. The second driver is the `GPIO`
driver, which pinmuxes (that is, routing signals from inside the `SoC` to actual HW pins) the RPi's
PL011 UART accordingly. Note how the `GPIO` driver differentiates between **RPi 3** and **RPi 4**.
Their HW is different, so we have to account for it in SW.
