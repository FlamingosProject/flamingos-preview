...
- [Tutorial 14 - Virtual Memory Part 2: MMIO Remap](#tutorial-14---virtual-memory-part-2-mmio-remap)
  ...
  - [Implementation](#implementation)
    - [Applying the Mapping API in `src/memory/mmu/translation_table.rs`](#applying-the-mapping-api-in-srcmemorymmutranslation_tablers)
...
Until now, the whole address space of the board was identity mapped at once. The generic memory layer
already supplied typed addresses, regions, attributes, and the `TranslationTable` interface, but the
**architecture** (`src/_arch/_/memory/**`) still orchestrated setup by querying **bsp**
(`src/bsp/_/memory/**`) policy directly.
...
### Applying the Mapping API in `src/memory/mmu/translation_table.rs`

Chapter 10 introduced the typed `TranslationTable` interface when translation tables first appeared.
This chapter changes who drives that interface: generic kernel code now creates selected mappings
incrementally instead of architecture code eagerly mapping the whole board:
...
- [`src/memory.rs`](kernel/src/memory.rs) and
  [`src/memory/mmu/types.rs`](kernel/src/memory/mmu/types.rs) extend the typed address, page, and
  memory-region vocabulary introduced in Chapter 10 for sparse mapping and MMIO allocation.
