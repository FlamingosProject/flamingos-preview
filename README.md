...
routine, provided by the `UART` driver, will run in response to the `IRQ` and drain pending receive
data without imposing a terminal echo policy in the raw driver.
...
In the handler, the UART acknowledges the interrupt and drains any pending receive bytes. The raw
driver does not echo input; terminal echo belongs to the terminal line discipline or higher-level
application code:
    ...
    fn handle(&self) -> Result<(), &'static str> {
        self.inner.lock(|inner| {
            ...
            if pending.any_matching_bits_set(MIS::RXMIS::SET + MIS::RTMIS::SET) {
                // Drain received characters. Echo policy belongs to a higher layer.
                while inner.read_byte(BlockingMode::NonBlocking).is_some() {}
            }
        });
...
When you load the kernel, UART receive interrupts are enabled and handled without a polling loop at
the end of `kernel_main()`. The current IRQ handler drains pending raw bytes; it does not echo them
back to the host:
