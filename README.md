...
The board-specific helper images are kept in `tools/jtag` so this workflow is available from a
numbered-branch checkout. The Chapter 06 chainloader also remains available: build a replacement
loader with `CHAINLOADER=1 make`, or send the normal kernel with `make chainboot`.
...
Install OpenOCD using your host package manager. The target configurations live in `tools/jtag`;
the interface configuration defaults to OpenOCD's
`interface/ftdi/olimex-arm-usb-tiny-h.cfg`. Override `OPENOCD_INTERFACE` for another adapter.
...
Finally, we need an `AArch64`-capable version of GDB, normally installed as `gdb-multiarch`. Override
the `GDB` Make variable if it has another name. Launch it with `make gdb`.

This Makefile target actually does a little more. It builds a special version of our kernel with
debug information included in an isolated `target/jtag` directory. This enables GDB to show Rust
source lines without replacing the normal release image. The startup file connects to core 0,
halts it, loads the ELF, and sets the program counter to `_start`.
