...
- The new `xtask` build frontend preserves caller-supplied Cargo arguments and merges feature flags
  with the selected board feature throughout the build pipeline.
...
The Rust tool for translation table computation is located in
`$ROOT/tools/translation_table_tool`. Its modules follow the kernel's split between
architecture-specific table construction, BSP-specific layout, generic memory types, and ELF
processing:
...
The architectural table builder necessarily mirrors some of
`_arch/aarch64/memory/mmu/translation_table.rs`. Keeping the host tool as a normal Rust crate makes
that correspondence type checked and keeps the build on one language and toolchain.

The new `cargo xtask build <board>` frontend invokes the tool after compiling and linking the kernel
and before generating the stripped binary. Arguments after the board name are forwarded to Cargo,
and caller features are merged with the required BSP feature into one consistent feature set. The
raw ELF build and final object-copy therefore operate on the same artifact. `make test_boot` uses an
isolated test build while still exercising the image with precomputed tables.

For example, a feature-gated build can be requested without editing the build scripts:

```console
$ cargo xtask build rpi3 --features=boot_trace
```

The tool parses the board and input ELF, verifies the architecture, derives the BSP layout, and
constructs an initially empty translation table:
...
let mut translation_tables = TranslationTable::new(
    bsp.kernel_virt_addr_space_size(),
    bsp.phys_addr_of_kernel_tables(&kernel_elf)?,
)?;
```

`KernelELF` uses the `goblin` crate to inspect load segments, section names, virtual and physical
addresses, and access flags. It converts each load segment into a mapping descriptor. Those
descriptors are then applied to the host-side table:
...
For this to work, the host-side `TranslationTable` needs the location and layout of the kernel's
table structure. The BSP implementation gets the location from the `KERNEL_TABLES` symbol in the
ELF. The architectural implementation defines the layout and must remain in sync with
`translation_table.rs` in the kernel.

Finally, after the mappings have been created, it is time to _patch_ them back into the kernel ELF
file:

```rust
kernel_patch_tables(&args.kernel_elf_path, &translation_tables, &bsp, &kernel_elf)?;
kernel_patch_base_addr(&args.kernel_elf_path, &translation_tables, &bsp, &kernel_elf)?;
```
