...
- Running from an SD card was a nice experience, but it would be extremely tedious to do it for
  every new binary. So let's write a [chainloader], selected in this and later chapters by the
  `chainloader` Cargo feature.
...
Our chainloader is called `MiniLoad` and is inspired by [raspbootin]. The host terminal and upload
client is the Rust program [`scip`]. Install it with `cargo install serial-console`.

[`scip`]: https://github.com/FlamingosProject/scip

You can try it with this tutorial already:
1. Depending on your target hardware, run `make` or `BSP=rpi4 make`. Chapter 06 enables the
   `chainloader` feature by default; `CHAINLOADER=1 make` is the explicit form used by later
   chapters.
...
In this tutorial, `CHAINBOOT_PAYLOAD` defaults to the kernel from the previous tutorial for the
demonstration. In subsequent tutorials, it defaults to the working directory's normal kernel image.
