...
pub fn generate_symbols(kernel_elf: &KernelELF, output_file: &str) -> Result<()> {
    ...
    // Write each symbol
    for sym in symbols {
        let name = demangle_symbol_name(&sym.name);
        writeln!(
            file,
            "    Symbol::new({}, {}, {:?}),",
            sym.value, sym.size, name
        )?;
    }
...
#[cfg(test)]
mod tests {
    use super::demangle_symbol_name;

    #[test]
    fn demangles_legacy_symbols_without_hashes() {
        let name = "_ZN4test4main17h1e2c3b4a59687786E";
        assert_eq!(demangle_symbol_name(name), "test::main");
    }

    #[test]
    fn demangles_v0_symbols() {
        let name = "_RNvCs9gjDRBXIxrx_6kernel11kernel_init";
        assert_eq!(demangle_symbol_name(name), "kernel::kernel_init");
    }

    #[test]
    fn preserves_non_rust_symbols() {
        assert_eq!(demangle_symbol_name("_start"), "_start");
    }
}
...
pub fn patch_symbol_data(kernel_elf: &KernelELF, symbols_blob_path: &str) -> Result<()> {
    ...
    let mut file = OpenOptions::new()
        ...
        .with_context(|| {
            format!(
                "Failed to open ELF file for patching: {}",
                kernel_elf.path()
            )
        })?;
