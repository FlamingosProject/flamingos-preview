use crate::Result;
use crate::memory::{
    AccessPermissions, AttributeFields, GRANULE_64KIB_SHIFT, GRANULE_512MIB_MASK,
    GRANULE_512MIB_SHIFT, MemoryAttributes, MemoryRegion,
};
use anyhow::anyhow;
...
impl TranslationTable {
    ...
    pub fn map_at(
        ...
        if virt_region.len() != phys_region.len() {
            return Err(anyhow!(
                "Virtual and physical regions must have the same size"
            ));
        }
        ...
        for (virt_page, phys_page) in virt_region.zip_with(phys_region) {
            ...
            Self::set_lvl3_entry(
                &mut self.lvl3_tables[lvl2_index][lvl3_index],
                phys_page,
                attributes,
            )?;
        }
