use crate::Result;
use crate::memory::{
    AccessPermissions, Alignment, AttributeFields, GRANULE_64KIB_SIZE, MemoryAttributes,
    MemoryRegion,
};
use anyhow::anyhow;
...
impl KernelELF {
    ...
    pub fn segment_containing_virt_addr(&self, virt_addr: u64) -> Option<&ProgramHeader> {
        self.elf
            .program_headers
            .iter()
            .find(|ph| ph.p_vaddr <= virt_addr && virt_addr < ph.p_vaddr + ph.p_memsz)
    }
    ...
    pub fn sections_in_segment(&self, segment: &ProgramHeader) -> String {
        ...
        let section_names: Vec<String> = self
            ...
            .filter_map(|sh| {
                if sh.sh_addr >= head
                    && sh.sh_addr < tail
                    && (sh.sh_flags & SECTION_FLAG_ALLOC) != 0
                {
                    self.elf
                        .shdr_strtab
                        .get_at(sh.sh_name)
    ...
    pub fn generate_mapping_descriptors(&self) -> Result<Vec<MappingDescriptor>> {
        ...
        for segment in self.select_load_segments() {
            ...
            let attributes =
                AttributeFields::new(MemoryAttributes::CacheableDRAM, acc_perms, execute_never);
...
impl fmt::Display for MappingDescriptor {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let name = format!(
            "{:<width$}",
            self.name,
            width = self.max_section_name_length
        );
        let virt_start = self
            .virt_region
            .first()
            .unwrap_or(0)
            .to_hex_underscore(true);
        let phys_start = self
            .phys_region
            .first()
            .unwrap_or(0)
